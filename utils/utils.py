...
async def is_admin_in_guild(ctx: Context[Bot], /):
    """Check whether a member is an admin in the server"""
    in_guild = next(
        (guild for guild in ctx.bot.guilds if guild.id == CONFIG.SERVER_ID), None
    )
    if not in_guild:
        return False
    compsoc_member = in_guild.get_member(ctx.message.author.id)
    if not compsoc_member:
        return False

    roles = [
        discord.utils.get(compsoc_member.roles, id=CONFIG.ADMIN_ROLE_ID)
    ]
    return any(roles)
